...
if (isset ($_REQUEST['lead_newsearch']) || empty ($_SESSION[$systemIdInSession]['lead_search'])) {
	ToolBox::formatUserPost($_REQUEST);
	$_SESSION[$systemIdInSession]['lead_search'] = array ();
	if (isset ($_REQUEST['lead_type']) && strcmp($_REQUEST['lead_type'],'-1')!=0) {
		$_SESSION[$systemIdInSession]['lead_search']['type'] = $_REQUEST['lead_type'];
	}
	if (isset ($_REQUEST['lead_source']) && strcmp($_REQUEST['lead_source'],'-1')!=0) {
		$_SESSION[$systemIdInSession]['lead_search']['source'] = $_REQUEST['lead_source'];
	}
	if (isset ($_REQUEST['lead_status']) && strcmp($_REQUEST['lead_status'],'-1')!=0) {
		$_SESSION[$systemIdInSession]['lead_search']['status'] = $_REQUEST['lead_status'];
	}
	$_SESSION[$systemIdInSession]['lead_search']['page_index'] = 1;
	$_SESSION[$systemIdInSession]['lead_search']['sort'] = 'Last created first';
}
if (!isset ($_SESSION[$systemIdInSession]['lead_search'])) {
	$_SESSION[$systemIdInSession]['lead_search'] = array ();
}
// critères de filtrage
$criteria = array ();
if (isset ($_SESSION[$systemIdInSession]['lead_search']['type'])) {
	if (empty($_SESSION[$systemIdInSession]['lead_search']['type'])) {
		$criteria['type'] = '';
	} else {
		$criteria['type'] = $_SESSION[$systemIdInSession]['lead_search']['type'];
	}
}
if (isset ($_SESSION[$systemIdInSession]['lead_search']['source'])) {
	if (empty($_SESSION[$systemIdInSession]['lead_search']['source'])) {
		$criteria['source'] = '';
	} else {
		$criteria['source'] = $_SESSION[$systemIdInSession]['lead_search']['source'];
	}
}
if (isset ($_SESSION[$systemIdInSession]['lead_search']['status'])) {
	$criteria['status'] = $_SESSION[$systemIdInSession]['lead_search']['status'];
}
...
if (isset ($_REQUEST['lead_search_page_index']))
	$_SESSION[$systemIdInSession]['lead_search']['page_index'] = $_REQUEST['lead_search_page_index'];

//	sélection de leads correspondant aux critères (dont le nombre dépend de la variable $page_items_nb)
$page_debut = ($_SESSION[$systemIdInSession]['lead_search']['page_index'] - 1) * $page_items_nb;
$page_rowset = $system->getLeadsRowset($criteria, $_SESSION[$systemIdInSession]['lead_search']['sort'], $page_items_nb, $page_debut);
...
<main class="container-fluid">
    	...
    	<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" class="form-inline">
    		<div class="form-group m-2">
        		...
        		<select id="lead_type_i" name="lead_type" class="form-control">
        			...
        			<?php echo isset($_SESSION[$systemIdInSession]['lead_search']['type']) ? Lead::getKnownTypesAsOptionsTags($_SESSION[$systemIdInSession]['lead_search']['type']) : Lead::getKnownTypesAsOptionsTags() ?>
        		</select>
    		</div>
    		<div class="form-group m-2">
        		...
        		<select id="lead_source_i" name="lead_source" class="form-control">
        			...
        			<?php echo isset($_SESSION[$systemIdInSession]['lead_search']['source']) ? Lead::getKnownSourcesAsOptionsTags($_SESSION[$systemIdInSession]['lead_search']['source']) : Lead::getKnownSourcesAsOptionsTags() ?>
        		</select>
        	</div>
        	<div class="form-group m-2">
        		...
        		<select id="lead_status_i" name="lead_status" class="form-control">
        			...
        			if (isset($_SESSION[$systemIdInSession]['lead_search']['status'])) {
        				$searchPattern->setStatus($_SESSION[$systemIdInSession]['lead_search']['status']);
        			}
	...
	<?php
		if ($pages_nb > 1) {
			$params = array ();
			echo ToolBox::getHtmlPagesNav($_SESSION[$systemIdInSession]['lead_search']['page_index'], $pages_nb, $params, 'lead_search_page_index');
		}
	?>
	</div>
	</section>
