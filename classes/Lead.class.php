...
class Lead {
	...
	public function getStatusOptionsTags($valueToSelect=NULL)	{
		$values = array('relevée', 'à suivre', 'suivie', 'abandonnée');
	...
	private static function getKnownTypes($label_substring = NULL) {
		$sql = 'SELECT lead_type AS value, COUNT(*) AS count FROM lead';
		$sql.= ' WHERE lead_type IS NOT NULL';
		if (isset($label_substring)) {
			$sql.= ' AND lead_type LIKE \'%'. mysql_real_escape_string($label_substring).'%\'';
		}
	...
	private static function getKnownSources($label_substring = NULL) {
		$sql = 'SELECT lead_source AS value, COUNT(*) AS count';
		$sql.= ' FROM lead';
		$sql.= ' WHERE lead_source IS NOT NULL';
		if (isset($label_substring)) {
			$sql.= ' AND lead_source LIKE \'%'. mysql_real_escape_string($label_substring).'%\'';
		}
	...
	public function toDB() {
		...
		if (!empty($this->society->id)) {
			$settings[] = 'society_id='.$this->society->id;
		}
		if ($this->getShortDescription()) {
			$settings[] = 'lead_shortdescription="'.mysql_real_escape_string($this->shortdescription).'"';
		}
		if (!empty($this->description)) {
			$settings[] = 'lead_description="'.mysql_real_escape_string($this->description).'"';
		}
		if (!empty($this->source_description)) {
			$settings[] = 'lead_source_description="'.mysql_real_escape_string($this->source_description).'"';
		}
		if (isset($this->type)) {
			$settings[] = 'lead_type="'.mysql_real_escape_string($this->getType()).'"';
		}
		if (isset($this->status)) {
			$settings[] = 'lead_status="'.mysql_real_escape_string($this->getStatus()).'"';
		}
		if (!empty($this->source)) {
			$settings[] = 'lead_source="'.mysql_real_escape_string($this->source).'"';
		}
		if ($new) {
			$settings[] = 'lead_creation_date=NOW()';
			$settings[] = 'lead_creation_user_id='.$_SESSION['user_id'];
		}
		else {
			$settings[] = 'lead_lastModification_user_id='.$_SESSION['user_id'];
		}
