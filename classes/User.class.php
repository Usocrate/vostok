...
class User {
	...
	public function toDB()
	{
		...
		$settings = array();
		if ($this->name) $settings[] = 'username=\''.mysql_real_escape_string($this->name).'\'';
		if ($this->password) $settings[] = 'password=\''.mysql_real_escape_string($this->password).'\'';		

		$sql = ($this->id)?'UPDATE':'INSERT INTO';
		$sql.= ' user SET ';
		$sql.= implode(', ',$settings);
		if ($this->id) $sql.= ' WHERE user_id='.$this->id;
		//echo '<p>'.$sql.'< /p>';
	...
	public function identification($name, $password)
	{
		$sql = 'SELECT *';
		$sql.= ' FROM user';
		$sql.= ' WHERE username=\''.mysql_real_escape_string($name).'\'';
