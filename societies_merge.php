	...
	<div class="pure-g-r">
		...
		<div class="pure-u-1">
			...
			<ul>
				...
				// print_r($s1);
				if ($s1->getCreationDate ())
					echo ' <small>(créée le ' . $s1->getCreationDate () . ')</small>';
				?>
				</li>
				<li><?php
				echo $s2->getNameForHtmlDisplay ();
				// print_r($s2);
				if ($s2->getCreationDate ())
					echo ' <small>(créée le ' . $s2->getCreationDate () . ')</small>';
				?>
				</li>
			</ul>
			<br />
			...
			<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" class="pure-form pure-form-stacked">
				...
				echo '<br />';
				?>
				<button type="submit" name="fusion_submission" value="1" class="pure-button pure-button-primary">Valider</button>
