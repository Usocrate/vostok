...
<body id="societiesListDoc" class="pure-skin-vostok">
	<div class="pure-g-r">
		...
		<div class="pure-u-1">
			<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" class="pure-form">
				<label for="s_name_i">nom</label> <input id="s_name_i" name="society_name" type="text" value="<?php echo $search_pattern->getName(); ?>" /> <label for="s_industry_i">activité</label> <select id="s_industry_i" name="industry_id">
				...
				</select> <label for="s_city_i">ville</label> <select id="s_city_i" name="society_city">
					<?php echo $search_pattern->getCityOptionsTags(); ?>
				</select> <label for="s_pc_i">code postal</label> <input id="s_pc_i" name="society_postalcode" type="text" size="5" value="<?php echo $search_pattern->getPostalCode(); ?>" />
				<button type="submit" name="society_newsearch" value="filtrer" class="pure-button">Filtrer</button>
			</form>
		</div>
		<div class="pure-u-1">
			<form method="post" action="societies_merge.php" class="pure-form">
				<table class="pure-table">
					...
					<tfoot>
						<tr>
							<td colspan="5" style="text-align: left">
								<button type="button" class="pure-button" onclick="check('society_id[]')">tout cocher</button> /
								<button type="button" class="pure-button" onclick="uncheck('society_id[]')">tout décocher</button> <label for="task_id">Pour la sélection :</label> <!--
					...
					-->
								<button name="task_submission" type="submit" value="1" class="pure-button pure-button-primary">fusionner</button> Créer une <a href="society_edit.php"><strong>nouvelle société</strong> </a>
							</td>
						</tr>
					</tfoot>
					<tbody>
